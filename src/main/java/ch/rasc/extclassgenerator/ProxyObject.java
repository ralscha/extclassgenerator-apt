...
public class ProxyObject {
	...
	protected ProxyObject(ModelBean model, OutputConfig config) {
		if (Util.hasText(model.getIdProperty()) && !model.getIdProperty().equals("id")) {
		...
		if (Util.hasText(model.getReader()) && !"json".equals(model.getReader())) {
		...
		if (Util.hasText(model.getRootProperty())) {
		...
		if (Util.hasText(model.getWriter()) && !"json".equals(model.getWriter())) {
		...
		if (config.getOutputFormat() == OutputFormat.EXTJS5
				...
				&& Util.hasText(model.getClientIdProperty())) {
		...
		if (Util.hasText(model.getCreateMethod())) {
			hasApiMethods = true;
			apiObject.create = model.getCreateMethod();
		}
		if (Util.hasText(model.getUpdateMethod())) {
			hasApiMethods = true;
			apiObject.update = model.getUpdateMethod();
		}
		if (Util.hasText(model.getDestroyMethod())) {
			hasApiMethods = true;
			apiObject.destroy = model.getDestroyMethod();
		}

		if (Util.hasText(model.getReadMethod())) {
			if (hasApiMethods) {
