...
public class ModelFieldBean {
	...
	public boolean hasOnlyName(OutputConfig outputConfig) {
		if (Util.hasText(this.name) && !Util.hasText(this.type)
				&& this.defaultValue == null && !Util.hasText(this.dateFormat)
				&& !Util.hasText(this.mapping) && this.persist == null
				&& !Util.hasText(this.convert)) {
